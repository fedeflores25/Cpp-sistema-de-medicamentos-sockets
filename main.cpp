...
int main()
{
    bool banderaLogin = true;
    int contador = 3;
    Cliente *cliente = new Cliente(5555, "192.168.0.105");
    ...
    while(banderaLogin)
    {
        ...
        cliente->enviar("l");// es una L en minuscula

        //respuesta si esta ocupado codigo n o tiene que ingresar contraseña codigo 2
        if(cliente->recibir() == "s")
        {
            ...
            if( (cliente->recibir() == "m") && (contador > 0))
            {

                cout<<"Usuario o contraseña incorrecta -  "<<contador<<" intentos restantes"<<endl;
                contador=contador-1;
            }
            else if( cliente->recibir() == "c")
            {
                cout<<"Puede acceder al menu"<<endl;
